...
| WX006 | 未配置正文文件 | 配置完整性 |
| WX007 | 正文文件不存在 | 配置完整性 |
| WX008 | 正文超过 2 万字符（按 HTML 长度，非纯文本）。计数前先剥离 HTML 注释与编辑器注入的记账属性（`data-page-node-id` 等），微信同样会过滤这两类内容 | 官方硬约束 |
...
| WX112 | 单个段落超过 250 字（代码卡片除外：`enhance_content.py` 重建的卡片用单个 `<p>` 承载整段代码，按 `font-family:…monospace` 识别并跳过） | 经验阈值 |
...
| WX116 | 封面分辨率偏低（宽 <1200px），放大显示会糊 | 经验阈值（900px 展示 ×2 屏=1800px） |
...
| WX218 | 正文图片分辨率偏低（宽 <750px），放大显示会糊 | 经验阈值（正文区 ~677px 宽 ×2 屏≈1354px） |
...
| WX210 | 行高偏紧（<1.6） |
| WX211 | 未选择风格预设 |
| WX212 | 封面分辨率未达 2x 高清（宽 <1800px） | 建议 1800×766（900px 展示 ×2） |
| WX213 | 正文图分辨率过大（宽 >2400px 且体积 >500KB） | 高清长图（如 3600px 宽 mermaid）不误报 |
